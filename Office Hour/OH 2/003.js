...
function calculate(arrNumber) {
  ...
  for(let i =0; i<arrNumber.length; i++){
    if(arrNumber[i].operator === '*'){
      result[i] = kali(arrNumber[i].operand1, arrNumber[i].operand2);
    } else if(arrNumber[i].operator === '-'){
      result[i] = kurang(arrNumber[i].operand1, arrNumber[i].operand2);
    } else if(arrNumber[i].operator === '+'){
      result[i] = tambah(arrNumber[i].operand1, arrNumber[i].operand2);
    } else if(arrNumber[i].operator === '/'){
      result[i] = bagi(arrNumber[i].operand1, arrNumber[i].operand2);
    }
  }
  return result;
}
